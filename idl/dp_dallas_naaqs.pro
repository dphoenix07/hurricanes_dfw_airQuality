...
;     file in dp_dpure coordinates for use in TRAJ3D.
;     W at the surface is set to zero.  One dp_dpure level is added at 
...
;     DIRECTORY : Output directory for dp_dp file.
...
infile = indir + 'dallas_dp_' + year + '.csv'
...
CASE year OF 
	'2000' : BEGIN
			site1 = WHERE(data.field03 EQ 1002)
   			site2 = WHERE(data.field03 EQ 3009)
         
         	dp1 = data.field14[site1]
            dp2 = data.field14[site2]
...
dp1_8hr = FLTARR(N_ELEMENTS(site1))
FOR tt = 0, N_ELEMENTS(site1)-9 DO BEGIN
	dp1_8hr[tt] = MEAN(dp1[tt:tt+8],/NAN)
ENDFOR
dp_8hr_total = dp1_8hr
dp_hourly = [dp_hourly, dp1]
...
FOR i = 0 , nt-1, 24 DO BEGIN
	indices=[]
   	FOR hh=0,23 DO BEGIN
   		...
   		indices = [indices, io3]
   	ENDFOR
	;Find daily max dp_dp
	dp_daily_max = [dp_daily_max, [MAX(dp_8hr_total[indices],/NAN), $
		MAX(dp_8hr_total[indices],/NAN), MAX(dp_8hr_total[indices],/NAN), $
		MAX(dp_8hr_total[indices],/NAN)]]
	dp_datestr = [dp_datestr, [time_arr[i],time_arr[i+6],time_arr[i+12],time_arr[i+18]]]
ENDFOR


RETURN, {dp_daily_max 		: dp_daily_max, $
				dp_datestr 	: dp_datestr, $
				dp_8hr_total : dp_8hr_total, $
				dp_hourly	: dp_hourly, $
				date            : data.field12, $
				time			: data.field13}
