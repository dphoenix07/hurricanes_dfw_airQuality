...
infile = indir + 'dallas_rh_' + year + '.csv'
...
CASE year OF 
	'2000' : BEGIN
			site1 = WHERE(data.field03 EQ 1002)
         	...
         	rh1 = data.field14[site1]
            rh2 = data.field14[site2]
...
rh1_8hr = FLTARR(N_ELEMENTS(site1))
FOR tt = 0, N_ELEMENTS(site1)-9 DO BEGIN
	rh1_8hr[tt] = MEAN(rh1[tt:tt+8],/NAN)
ENDFOR
rh_8hr_total = rh1_8hr
rh_hourly = [rh_hourly, rh1]
...
FOR i = 0 , nt-1, 24 DO BEGIN
	...
	rh_daily_max = [rh_daily_max, [MAX(rh_8hr_total[indices],/NAN), $
		MAX(rh_8hr_total[indices],/NAN), MAX(rh_8hr_total[indices],/NAN), $
		MAX(rh_8hr_total[indices],/NAN)]]
	rh_datestr = [rh_datestr, [time_arr[i],time_arr[i+6],time_arr[i+12],time_arr[i+18]]]
ENDFOR


RETURN, {rh_daily_max 		: rh_daily_max, $
				rh_datestr 	: rh_datestr, $
				rh_8hr_total : rh_8hr_total, $
				rh_hourly	: rh_hourly, $
				date            : data.field12, $
				time			: data.field13}
